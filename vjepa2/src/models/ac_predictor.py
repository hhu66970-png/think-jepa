...
class VisionTransformerPredictorAC(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.action_encoder = nn.Linear(
            action_embed_dim, predictor_embed_dim, bias=True
        )
        # self.state_encoder = nn.Linear(action_embed_dim, predictor_embed_dim, bias=True)
        self.state_encoder = nn.Linear(12, predictor_embed_dim, bias=True)
    ...
    def forward(self, x, actions, states, extrinsics=None):
        ...
        # print(f"states: {states.shape}, state_encoder {self.state_encoder}")
        # print(f"actions: {actions.shape}, action_encoder {self.action_encoder}")        # 写法1（最直白）
        # actions = actions.contiguous().view(B, T, -1)  # [32, 1, 180]
        ...
        #############################
        a = a.repeat(1, s.size(1), 1, 1)  # -> [32,  15,1, 1024]
        #############################
        x = x.view(B, T, self.grid_height * self.grid_width, D)  # [B, T, H*W, D]

        # print([a.shape, s.shape, x.shape])
        ...
        attn_mask = self.attn_mask[: x.size(1), : x.size(1)].to(
        ...
        x = x.view(
            B, T, cond_tokens + self.grid_height * self.grid_width, D
        )  # [B, T, K+H*W, D]
